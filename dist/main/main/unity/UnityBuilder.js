...
`;
                    ...
                    this.emit('log', `[Arsist] Warning: failed to update link.xml: ${err.message}`);
                }
            }
            // Phase 4: Unityビルド実行
    ...
    async resolveUniVRMUnityPackagePath() {
    ...
    async importUnityPackage(projectPath, packagePath, logFile) {
        return new Promise((resolve) => {
            const args = [
                '-batchmode',
                '-nographics',
     ...
     */
    async ensureJintDlls(unityProjectPath) {
        ...
        if (await fs.pathExists(oldEsprima)) {
            ...
            if (await fs.pathExists(oldEsprimaMeta))
                await fs.remove(oldEsprimaMeta);
    ...
    async extractFromZip(zipPath, destDir) {
        await fs.ensureDir(destDir);
        return new Promise((resolve, reject) => {
            let proc;
            if (process.platform === 'win32') {
                const script = `Expand-Archive -LiteralPath '${zipPath}' -DestinationPath '${destDir}' -Force`;
                proc = (0, child_process_1.spawn)('powershell.exe', ['-NoProfile', '-NonInteractive', '-Command', script], { shell: false, windowsHide: true });
            }
            else {
                // Linux / macOS: unzip が無ければ python3 にフォールバック
                proc = (0, child_process_1.spawn)('unzip', ['-o', '-q', zipPath, '-d', destDir], { shell: false });
            }
            const stderr = [];
    ...
    async transferProjectData(unityProjectPath, config) {
        ...
        if (config.scriptsData) {
            await fs.writeJSON(path.join(dataDir, 'scripts.json'), config.scriptsData, { spaces: 2 });
