...
2. **リソース制御:** IL2CPP環境では Jint のメモリトラッキングが正しく動作しないため、**メモリ上限は設定せず**、再帰深さ (`LimitRecursion`) と実行タイムアウト (`TimeoutInterval`) の2軸で無限ループや暴走を防ぎます。
...
public class ScriptEngineManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        _engine = new Engine(cfg => cfg
            // NOTE: IL2CPP では LimitMemory が不正値を返すため無効化
            .LimitRecursion(64)     // 再帰呼び出しの深さ制限
            .TimeoutInterval(System.TimeSpan.FromSeconds(5)) // 実行タイムアウト(無限ループ対策)
        );
...
1. Jintの初期化設定で **`LimitRecursion` と `TimeoutInterval`** を厳格に設定する（IL2CPPでは `LimitMemory` を使用しない）。
