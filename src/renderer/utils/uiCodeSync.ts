...
function elementToHtml(element: UIElement, isRoot = false): string {
  ...
  if (element.bind?.key) attrs.push(`data-arsist-bind="${escapeAttribute(element.bind.key)}"`);
  if (element.bind?.format) attrs.push(`data-arsist-format="${escapeAttribute(element.bind.format)}"`);
...
function parseElement(element: Element): UIElement {
  ...
  const bindKey = element.getAttribute('data-arsist-bind');
  const bindFormat = element.getAttribute('data-arsist-format');
  ...
  return {
    ...
    content,
    children,
    bind: bindKey ? { key: bindKey, format: bindFormat || undefined } : undefined,
  };
}
