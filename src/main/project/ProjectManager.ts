...
`;
   ...
   */
  validateHTML(htmlFragment: string, css: string, js: string): { valid: boolean; issues: string[]; warnings: string[] } {
    const issues: string[] = [];
    ...
    if (js && js.length > 0) {
      try {
        ...
        warnings.push(`JavaScript解析時のエラー: ${(e as Error).message}`);
      }
    }
   ...
   * HTMLの要素ネスト深度を計算
   */
  private calculateHTMLDepth(html: string): number {
    let maxDepth = 0;
    let currentDepth = 0;
