 ...
 */
export function convertSceneToUnity(scene: SceneData): UnitySceneData {
  return {
    name: scene.name,
...
interface UnityGameObject {
  ...
  transform: {
    ...
    localRotation: Vector3; // Euler angles
    localScale: Vector3;
  };
...
function convertObjectToUnity(obj: SceneObject): UnityGameObject {
...
function getPrimitiveMeshName(primitiveType?: string): string {
 ...
 */
export function convertUIToUnity(layout: UILayoutData): UnityUIData {
  return {
    name: layout.name,
...
interface UnityUIElement {
  ...
  image?: {
    ...
    material?: string; // For blur effect
  };
  text?: {
    text: string;
    fontSize: number;
  ...
  button?: {
    ...
    colors: {
      ...
      highlightedColor: { r: number; g: number; b: number; a: number };
...
function convertUIElementToUnity(element: UIElement): UnityUIElement {
  const result: UnityUIElement = {
...
function calculateRectTransform(element: UIElement): UnityUIElement['rectTransform'] {
...
function mapAlignment(justify?: string, align?: string): string {
...
function mapTextAlignment(align?: string): string {
  switch (align) {
 ...
 * ロジックグラフをC#コードに変換
 */
export function convertLogicToCode(graphs: LogicGraphData[], projectName: string): string {
  let code = `// ==============================================
...
function generateClassFromGraph(graph: LogicGraphData): string {
...
function generateEventMethod(eventNode: LogicNode, graph: LogicGraphData): string {
  const unityEventName = mapEventToUnity(eventNode.eventType || '');
  if (!unityEventName) return '';
...
function mapEventToUnity(eventType: string): string {
  switch (eventType) {
...
function generateActionCall(node: LogicNode): string {
...
function generateActionMethod(node: LogicNode): string {
